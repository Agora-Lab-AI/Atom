...
def _ntk_find_correction_factor(
...
def _ntk_find_correction_range(
    low_rot, high_rot, dim, base=10000, max_position_embeddings=2048
):
    low = math.floor(
        _ntk_find_correction_factor(low_rot, dim, base, max_position_embeddings)
    )
    high = math.ceil(
        _ntk_find_correction_factor(high_rot, dim, base, max_position_embeddings)
    )
...
def _ntk_build_inv_freq(
    ...
    inv_freq_linear = 1.0 / (
        scaling_factor * (base ** (torch.arange(0, dim, 2).float().to(device) / dim))
    )
    inv_freq_ntk = 1.0 / (
        _ntk_find_newbase_ntk(dim, base, scaling_factor)
    ...
    low, high = _ntk_find_correction_range(
        beta_0, beta_1, dim, base, original_max_position_embeddings
    )
    inv_freq_mask = (
        1
        - _ntk_linear_ramp_mask(low, high, dim // 2)
        .type(current_dtype)
        .to(current_device)
    ) * ntk_factor
    ...
    low, high = _ntk_find_correction_range(
        gamma_0, gamma_1, dim, base, original_max_position_embeddings
    )
    inv_freq_mask = (
        1
        - _ntk_linear_ramp_mask(low, high, dim // 2)
...
def compute_flash_attention_inference(
    query_states, key_states, value_states, attention_mask=None, dropout=0.0
):
    ...
    if attention_mask is not None:
        ...
        query_states = torch.cat(
            [query_states[i, starts[i] : ends[i]] for i in range(batch)], dim=0
        )
        key_states = torch.cat(
            [key_states[i, starts[i] : ends[i]] for i in range(batch)], dim=0
        )
        value_states = torch.cat(
            [value_states[i, starts[i] : ends[i]] for i in range(batch)], dim=0
        )
    ...
    def forward(
        ...
        if self.use_flash_attention and not output_attentions:
            ...
            if self.training or query_states.shape == key_states.shape:
                ...
                attn_output = compute_flash_attention_packed(
                ...
                attn_output = compute_flash_attention_inference(
